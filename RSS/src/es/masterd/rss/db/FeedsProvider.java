...
public class FeedsProvider extends ContentProvider {
	...
	public int delete(Uri uri, String where, String[] whereargs) {
		...
		case POST_ID:
			...
			count = feedsDB.delete(Posts.NOMBRE_TABLA,Posts._ID+ " = "+ id+ (!TextUtils.isEmpty(where) ? " AND (" + where+ ')' : ""), whereargs);
			break;
		default:
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		case POST:
			...
			break;
		case POST_ID:
			count = feedsDB.update(Posts.NOMBRE_TABLA, values,Posts._ID+ " = "+ uri.getPathSegments().get(1)+ (!TextUtils.isEmpty(selection) ? " AND ("+ selection + ')' : ""), selectionArgs);
			break;
		default:
			throw new IllegalArgumentException("Unknown URI " + uri);
		}
